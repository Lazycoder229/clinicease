...
/* ─── Get doctor info ─── */
$me = $db->table('users u')
    ->select('CONCAT(p.first_name, \' \', p.last_name) AS full_name, u.email, u.role')
...
<head>
  ...
  <title>Settings — ClinicEase</title>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .settings-item { display: flex; align-items: center; justify-content: space-between; padding: 16px; border: 1px solid var(--border); border-radius: 10px; margin-bottom: 12px; }
    .settings-item-info h4 { font-size: 13px; font-weight: 700; color: var(--navy); }
    .settings-item-info p { font-size: 12px; color: var(--muted); margin-top: 4px; }
    .toggle-switch { width: 50px; height: 28px; border-radius: 14px; background: var(--border); cursor: pointer; position: relative; transition: background .2s; }
    .toggle-switch.active { background: var(--teal); }
    .toggle-switch::after { content: ''; position: absolute; width: 24px; height: 24px; border-radius: 50%; background: white; top: 2px; left: 2px; transition: left .2s; }
    .toggle-switch.active::after { left: 24px; }

    .form-group { margin-bottom: 18px; }
    .form-label { display: block; font-size: 12px; font-weight: 700; color: var(--navy); margin-bottom: 6px; text-transform: uppercase; letter-spacing: .5px; }
    .form-input { width: 100%; padding: 10px 12px; border: 1px solid var(--border); border-radius: 8px; font-family: 'DM Sans', sans-serif; font-size: 13px; color: var(--navy); background: var(--surface); transition: border-color .2s; }
    ...
    .btn-danger { padding: 10px 18px; background: #ef4444; color: #fff; border: none; border-radius: 8px; font-weight: 600; cursor: pointer; font-size: 13px; transition: background .2s; }
    ...
    @media (max-width: 768px) {
      ...
      .settings-item-info { width: 100%; margin-bottom: 12px; }
    }
  </style>
</head>
<body>
...
<main class="main">
  ...
  <div class="content">
    ...
    <div class="role-badge fade-up d1">
      <i class="fa-solid fa-sliders"></i>Settings — Configure your preferences
    </div>

    <!-- Notification Settings -->
    <div class="section fade-up d2">
      <div class="section-title">
        <i class="fa-solid fa-bell" style="color:var(--accent);"></i>Notification Settings
      </div>
    ...
    <div class="section fade-up d2">
      <div class="section-title">
        <i class="fa-solid fa-lock" style="color:var(--accent);"></i>Privacy & Security
      </div>
      ...
      <div style="margin-top:16px;padding-top:16px;border-top:1px solid var(--border);">
        <h4 style="font-size:13px;font-weight:700;color:var(--navy);margin-bottom:12px;">Change Password</h4>
    ...
    <div class="section fade-up d3">
      <div class="section-title">
        <i class="fa-solid fa-user-circle" style="color:var(--accent);"></i>Account Management
      </div>

      <div style="padding:16px;background:var(--surface);border-radius:10px;border-left:3px solid var(--accent);margin-bottom:16px;">
        <p style="font-size:12px;color:var(--navy);">Account Status: <span style="font-weight:700;">Active</span></p>
        <p style="font-size:12px;color:var(--muted);margin-top:4px;">Last login: Today at 2:30 PM</p>
      </div>

      <div style="display:flex;gap:10px;flex-wrap:wrap;">
        <button class="btn-primary" onclick="alert('Logout feature will be implemented')">
          <i class="fa-solid fa-sign-out-alt" style="margin-right:6px;"></i>Logout
        </button>
        <a href="<?= url('doctor/profile') ?>" class="btn-primary" style="background:var(--surface);color:var(--navy);border:1px solid var(--border);text-decoration:none;">
          <i class="fa-solid fa-user" style="margin-right:6px;"></i>My Profile
        </a>
        <button class="btn-danger" onclick="if(confirm('Are you sure you want to delete your account? This cannot be undone.')) { alert('Account deletion feature will be implemented'); }">
